...
import { BATTLE_MAP, LangCode, MapType, SALMONRUN_MAP } from "./configs.ts";
  ...
  const data = mapInfoJson
    .map(
      (mapInfo) => {
        ...
        let code: string;
        if (/\d$/.test(raw_code)) { // 숫자로 끝나는지 검사
...
function preprocessSalmonrunMapInfo(
  ...
  const data = mapInfoJson
    .filter((mapInfo) =>
      (mapInfo as Record<string, boolean>)["IsBigRun"] === false
    ) // 빅런은 제외
    .map(
      (mapInfo) => {
        mapInfo = mapInfo as { [key: string]: JsonType };
...
export async function main(labelDataJsonByLangCode: Map<LangCode, JsonType>) {
  ...
  await Promise.all(
    [
      _generateConstantOutput(BATTLE_MAP),
      _generateConstantOutput(SALMONRUN_MAP),
    ],
  );
}
